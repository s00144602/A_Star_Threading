...
void NPC::findPath()
{
	...
	m_targetCell = m_map.at(Point(20, 20));
...
void NPC::render(Renderer & r)
{
	...
	r.drawCircle(
		(m_pos.first*m_radius*2)+ m_radius - Camera::Instance()->getPosition().x,
		(m_pos.second*m_radius*2)+ m_radius - Camera::Instance()->getPosition().y,
		m_radius,
		{ 0,0,0 });
	...
	{
		r.drawCircle(
			(t->getPos().first) + m_radius - Camera::Instance()->getPosition().x,
			(t->getPos().second) + m_radius - Camera::Instance()->getPosition().y,
			m_radius,
			{ 0,0,255 });
	}
}
